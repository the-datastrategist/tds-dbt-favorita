...
class TestListRunConfigNames:
    ...
    def test_predict_step_excludes_train_configs(self) -> None:
...
class TestRunBatch:
    def test_resolve_single_config(self) -> None:
        assert resolve_batch_config_names("favorita_rf_train") == ["favorita_rf_train"]
    ...
    def test_resolve_empty_raises(self) -> None:
        with pytest.raises(ValueError, match="include_in_run"):
            resolve_batch_config_names(None, step="predict")

    @patch("vertex.jobs.run_batch.subprocess.run")
    def test_docker_batch_runs_all_configs(self, mock_run: MagicMock) -> None:
        mock_run.return_value = MagicMock(returncode=0)
        run_configs(
            ["favorita_xgboost_train", "favorita_rf_train"],
            vertex_mode="docker",
        )
    ...
    def test_docker_batch_raises_on_failure(self, mock_run: MagicMock) -> None:
        ...
        with pytest.raises(RuntimeError, match="Batch run failed"):
            run_configs(["favorita_xgboost_train"], vertex_mode="docker")
    ...
    def test_vertex_batch_submits_all(self, mock_submit: MagicMock) -> None:
        ...
        run_configs(["favorita_xgboost_train", "favorita_rf_train"], vertex_mode="vertex")
    ...
    def test_vertex_batch_sync_waits_for_all(self, mock_submit: MagicMock) -> None:
        ...
        run_configs(["favorita_xgboost_train"], vertex_mode="vertex", sync=True)
