"""Generate Prophet predictions with unified BigQuery rows.

Populates prediction_lower/prediction_upper from Prophet's native yhat_lower/yhat_upper —
already in the favorita_model_predictions DDL, unused by ARIMA/SARIMA, which don't emit
uncertainty intervals today.
"""
...
from vertex.models.prophet.prophet_common import predict_forward_rows, predict_holdout_rows
...
def run_predict_prophet(config: dict[str, Any]) -> dict[str, Any]:
    spec = get_job_spec(config)
    model_type = spec["model_type"]
    if model_type not in SUPPORTED_TYPES:
        raise ValueError(f"predict_prophet supports {sorted(SUPPORTED_TYPES)}; got {model_type!r}")
    ...
    if "prediction_lower" in scored.columns:
        prediction_rows["prediction_lower"] = scored["prediction_lower"].values
    if "prediction_upper" in scored.columns:
        prediction_rows["prediction_upper"] = scored["prediction_upper"].values
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Predict with Prophet")
    parser.add_argument("--config-path", "-f", default=str(DEFAULT_CONFIG_PATH))
    parser.add_argument("--config-name", "-c", required=True)
    args = parser.parse_args()
    run_predict_prophet(load_model_config(args.config_name, args.config_path))
