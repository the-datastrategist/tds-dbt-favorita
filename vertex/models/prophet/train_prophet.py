"""Train per-entity Prophet models and publish artifacts to GCS + BigQuery."""
...
def run_train_prophet(config: dict[str, Any]) -> dict[str, Any]:
    spec = get_job_spec(config)
    model_type = spec["model_type"]
    if model_type not in SUPPORTED_TYPES:
        raise ValueError(f"train_prophet supports {sorted(SUPPORTED_TYPES)}; got {model_type!r}")
    ...
    params, params_provenance = resolve_model_parameters(config, default_model_params())
    ...
    run_at = dt.utcnow()
    model_id = bundle_model_id(model_type, params, entities_fitted)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train Prophet from model_config.yaml")
    ...
    run_train_prophet(load_model_config(args.config_name, args.config_path))
