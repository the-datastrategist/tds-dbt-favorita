"""Optuna search over Prophet's changepoint_prior_scale (per-entity aggregate objective).

Scoped to changepoint_prior_scale only for the first cut — growth and seasonality_mode stay
fixed at their additive-seasonality defaults (see docs/specs/prophet_model_family.md
Non-goals: multiplicative seasonality is a fast-follow, not part of this search space).
"""
...
from vertex.models.prophet.prophet_common import default_model_params, fit_entity_models
from vertex.models.timeseries.ts_common import bundle_model_id, prepare_panel
...
SUPPORTED_TYPES = frozenset({"prophet"})


def suggest_prophet_params(trial: optuna.Trial) -> dict[str, Any]:
    return {
        "growth": "linear",
...
def run_optimize_prophet(config: dict[str, Any]) -> dict[str, Any]:
    spec = get_job_spec(config)
    model_type = spec["model_type"]
    if model_type not in SUPPORTED_TYPES:
        raise ValueError(f"optimize_prophet supports {sorted(SUPPORTED_TYPES)}; got {model_type!r}")
    ...
    def objective(trial: optuna.Trial) -> float:
        params = suggest_prophet_params(trial)
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Optimize Prophet changepoint_prior_scale")
    parser.add_argument("--config-path", "-f", default=str(DEFAULT_CONFIG_PATH))
    parser.add_argument("--config-name", "-c", default="favorita_store_n1d_prophet")
    args = parser.parse_args()
    config = load_model_config(args.config_name, args.config_path, step="optimize")
    result = run_optimize_prophet(config)
