"""Generate Random Forest predictions and write unified rows to BigQuery."""
...
def prepare_prediction_features(
    ...
    *,
    ...
    matrix, _features, _ = prepare_feature_matrix(
        work,
...
def run_predict_random_forest(config: dict[str, Any]) -> dict[str, Any]:
    spec = get_job_spec(config)
    if spec["model_type"] != "random_forest":
        raise ValueError(
            f"predict_random_forest does not support model_type={spec['model_type']!r}"
        )
    ...
    model_type = manifest.get("model_type", "random_forest")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Predict with Random Forest")
    parser.add_argument("--config-path", "-f", default=str(DEFAULT_CONFIG_PATH))
    parser.add_argument("--config-name", "-c", required=True)
    args = parser.parse_args()
    run_predict_random_forest(load_model_config(args.config_name, args.config_path))
