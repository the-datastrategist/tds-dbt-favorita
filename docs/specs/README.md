...
| [Forecast contract and canonical output](forecast_contract_and_output.md) | Shipped | 100% | Platform roadmap → P0 forecast contract | Live-accepted contract validation, canonical persistence, provenance, lifecycle publication, staging, and append-only migration boundary |
...
| [Point-in-time feature availability](point_in_time_feature_availability.md) | In progress | 90% | Platform roadmap → P0 feature correctness | Origin-specific cutoff enforcement, registry validation, persisted cutoff evidence, and model-path integration are implemented; live acceptance and accelerator graduation remain |
| [Forecasting methods, horizons, cold start, and intermittent demand](forecasting_methods.md) | In progress | 90% | Platform roadmap → P0/P1 methods | Horizon-aware models, intermittent-demand baselines, calibration, cold-start routing, and scheduled-stage integration are implemented; live end-to-end publication acceptance remains |
...
| [Scheduled forecast publication pipeline](scheduled_forecast_publication_pipeline.md) | In progress | 90% | Cross-spec operational integration | Deterministic champion scoring, routing, calibration, reconciliation gates, leases, failure evidence, Prefect deployment, and atomic draft visibility are implemented; live draft-only and hierarchical acceptance remain |
| [Monitoring, alerts, and SLOs](monitoring_and_slos.md) | Proposed | 35% | Platform roadmap → P1 monitoring | Define forecast freshness, completeness, accuracy, drift, calibration, pipeline, and cost monitoring |
