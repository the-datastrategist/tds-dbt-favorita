...
| [Monitoring, alerts, and SLOs](monitoring_and_slos.md) | In progress | 97% | Platform roadmap → P1 monitoring | Live-accepted source, pipeline, publication-freshness, prediction-coverage, feature-completeness, realized-calibration, and target/feature-drift signals plus an opt-in hosted evaluator and Cloud Monitoring routing are implemented; production activation and cost monitoring remain |
| [Integration contracts and forecast delivery](integration_contracts.md) | In progress | 90% | Platform roadmap → P2 integrations | Stable views, GCS export, events, delivery confirmation, and tested read-only retrieval API are implemented; live API deployment, mutations, and outbound webhooks remain |
