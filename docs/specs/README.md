...
| [General-purpose demand forecasting platform](platform_generalization.md) | In progress | 50% | Separate platform-generalization workstream | Canonical adapters and identity are live accepted across rolling-origin scoring, publication, persistence, reconciliation, retrieval, monitoring, and SLOs; all model-family configs now use canonical roles, while live family retraining, hierarchy adapters, period abstraction, and extension APIs remain |
