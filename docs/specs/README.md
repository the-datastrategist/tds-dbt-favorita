...
| [Workload Identity Federation](workload_identity_federation.md) | In progress | `iac.md` → security checklist "prefer WIF" | Terraform resources and GitHub Actions authentication are implemented; live-environment configuration and CI validation remain |
...
| [Point-in-time feature availability](point_in_time_feature_availability.md) | In progress | `demand_forecasting_platform_recommendations.md` → P0 feature correctness | Origin-specific cutoff enforcement, registry validation, persisted cutoff evidence, and model-path integration are implemented; live acceptance and accelerator graduation remain |
| [Forecasting methods, horizons, cold start, and intermittent demand](forecasting_methods.md) | In progress | `demand_forecasting_platform_recommendations.md` → P0/P1 methods | Horizon-aware models, intermittent-demand baselines, probabilistic calibration, cold-start fallbacks, and strategy routing are implemented; complete scheduled publication-path validation remains |
| [Hierarchical reconciliation](hierarchical_reconciliation.md) | In progress | `demand_forecasting_platform_recommendations.md` → P1 reconciliation | Configurable hierarchies, bottom-up/top-down/middle-out/MinT methods, persistence, and tests are implemented; scheduled publication integration remains |
...
| [Forecast operations](forecast_operations.md) | In progress | `demand_forecasting_platform_recommendations.md` → P1/P2 operations | Append-only override, approval, publication, revision, and exception tables plus staging contracts are implemented; workflow services and transition commands remain |
