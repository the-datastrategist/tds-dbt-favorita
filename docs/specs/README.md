...
| [Demand data model](demand_data_model.md) | In progress | 60% | Platform roadmap → P1 demand data | Explicit observed-sales proxy, canonical store-day demand semantics, eligibility decisions/reasons, summaries, tests, and pinned pipeline population are implemented; row-level run evidence and live optional-source adapters remain |
| [Forecast operations](forecast_operations.md) | In progress | 95% | Platform roadmap → P1/P2 operations | Append-only tables, atomic drafts, audited overrides, approval/publication, rollback lineage, delivery confirmation, and live operations acceptance are implemented; UI and FVA remain |
...
| [Monitoring, alerts, and SLOs](monitoring_and_slos.md) | In progress | 90% | Platform roadmap → P1 monitoring | Live-accepted source, pipeline, publication-freshness, prediction-coverage, and feature-completeness signals plus an opt-in hosted evaluator and Cloud Monitoring routing are implemented; production activation, drift, calibration, and cost remain |
| [Integration contracts and forecast delivery](integration_contracts.md) | In progress | 80% | Platform roadmap → P2 integrations | Stable forecast and audit views, version-pinned GCS export, publication events, and append-only delivery confirmation are live accepted; APIs and outbound webhooks remain |
