...
Status reviewed **2026-08-24**. The completion figures are implementation estimates, while
...
| [Demand data model](demand_data_model.md) | In progress | 95% | Platform roadmap → P1 demand data | Explicit observed-sales proxy, canonical store-day demand semantics, opt-in inventory/assortment/lifecycle/price/promotion/closure/external-demand adapters, immutable eligibility evidence, population gates, monitoring, and controlled exclusion acceptance are implemented; client-specific source mapping and live acceptance remain |
...
| [General-purpose demand forecasting platform](platform_generalization.md) | In progress | 80% | Separate platform-generalization workstream | Canonical adapters and identity are live accepted across core consumers; all six migrated model families have successful immutable-image Vertex retraining, full weekly/monthly pipeline acceptance is automated, and production dispatch resolves configured model providers. Live non-daily source deployment and project-specific optional-provider execution remain |
